...
const revenueOverTime: RequestHandler = asyncHandler(async (req, res) => {
    const days = Number(req.query.days) || 30;
    const since = daysAgo(days);
...
const ordersByStatus: RequestHandler = asyncHandler(async (req, res) => {
    const rows = await orderModel.aggregate([
...
const topProducts: RequestHandler = asyncHandler(async (req, res) => {
    const days = Number(req.query.days) || 30;
