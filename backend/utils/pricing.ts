...
const priceCartServerSide = async (items: CartLineInput[]): Promise<PricedCart> => {
    ...
    for (const it of items) {
        ...
        if (!Number.isInteger(quantity) || quantity <= 0) {
            throw new AppError(400, "Invalid quantity in cart.");
        }
        const product = await productModel.findById(it.productId);
        if (!product) throw new AppError(404, `Product not found: ${it.productId}`);
        const variantEntry = product.variants.find((v) => v.value === it.variant);
        if (!variantEntry) throw new AppError(400, `${it.variant} is not available for ${product.name}.`);
...
const applyCoupon = async (code: string | undefined | null, userId: string, itemsAmount: number): Promise<CouponApplication> => {
    ...
    if (itemsAmount < coupon.minOrderAmount) throw new AppError(400, `This coupon needs a minimum order of ${coupon.minOrderAmount}.`);
    if (coupon.maxUses != null && coupon.usedCount >= coupon.maxUses) throw new AppError(400, "This coupon has reached its usage limit.");
    ...
    if (priorUses >= coupon.perUserLimit) throw new AppError(400, "You have already used this coupon.");
    ...
    try {
        await session.withTransaction(async () => {
            for (const it of items) {
                ...
                if (!updated) {
                    throw new AppError(400, `Insufficient stock for ${it.variant}.`);
                }
            }
        });
    } finally {
