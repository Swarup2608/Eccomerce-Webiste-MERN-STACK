...
export default function ProductDetail() {
  ...
  const handleAdd = () => {
    ...
    for (let i = 0; i < qty; i++) addToCart(product._id, variant);
  };
        ...
        <div>
          ...
          <div style={{ margin: '26px 0 18px' }}>
            <div style={{ font: '500 11px/1 Inter', letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--color-neutral-500)', marginBottom: 10 }}>
              Select {product.filterLabel.toLowerCase()}
            </div>
              ...
              {product.variants.map((v) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={outOfStock}
                    onClick={() => { setVariant(v.value); setVariantError(false); }}
                    className="btn"
                    title={outOfStock ? 'Out of stock' : `${v.stock} left`}
                    style={{
                      border: `1px solid ${variant === v.value ? 'var(--color-accent)' : 'var(--color-divider)'}`,
                      color: variant === v.value ? 'var(--color-accent)' : 'var(--color-text)',
                      minWidth: 44,
            ...
            {variantError && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 10, fontSize: 12.5, color: 'var(--color-accent-2-300)' }}>
                Pick a {product.filterLabel.toLowerCase()} before adding to the cart.
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 26px' }}>
            {[
              ...
              [product.filterLabel, product.variants.map((v) => v.value).join(', ') || '—'],
