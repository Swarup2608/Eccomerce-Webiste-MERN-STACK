...
export default function PlaceOrder() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const orderItems = rows.map((r) => ({ productId: r.product._id, variant: r.variant, quantity: r.qty }));
      const orderData = {
        address: formData,
        items: orderItems,
      ...
      <main className="container" style={{ padding: '44px 26px 80px' }}>
        ...
        <div className="checkout-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.35fr) minmax(0,1fr)', gap: 40, alignItems: 'start' }}>
          ...
          <aside className="card" style={{ padding: 22, position: 'sticky', top: 96 }}>
            ...
            {rows.map((r) => (
              <div key={`${r.product._id}-${r.variant}`} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '9px 0' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--color-neutral-500)' }}>{r.product.filterLabel} {r.variant} × {r.qty}</div>
                </div>
