...
export default function Cart() {
  ...
  const rows = useMemo(() => {
    const out: { product: (typeof products)[number]; variant: string; qty: number }[] = [];
    for (const id in cartItems) {
      const product = products.find((p) => p._id === id);
      if (!product) continue;
      for (const variant in cartItems[id]) {
        if (cartItems[id][variant] > 0) out.push({ product, variant, qty: cartItems[id][variant] });
      }
    }
    return out;
  }, [cartItems, products]);
    ...
    <main className="container" style={{ padding: '44px 26px 80px' }}>
      ...
      {rows.length === 0 ? (
        ...
        <div className="checkout-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.6fr) minmax(0,1fr)', gap: 34, alignItems: 'start' }}>
          <div>
            {rows.map((r) => (
              <div key={`${r.product._id}-${r.variant}`} style={{ display: 'grid', gridTemplateColumns: '96px minmax(0,1fr) auto', gap: 18, alignItems: 'center', padding: '18px 0', boxShadow: '0 1px 0 var(--color-divider)' }}>
                <ProductThumb image={r.product.image?.[0]} name={r.product.name} />
                <div>
                  <div style={{ font: '500 16px/1.25 Inter' }}>{r.product.name}</div>
                  <div style={{ fontSize: 12.5, color: 'var(--color-neutral-500)', marginTop: 4 }}>{r.product.filterLabel} {r.variant} · {currency}{r.product.price}</div>
                  <button
                    type="button"
                    onClick={() => updateQuantity(r.product._id, r.variant, 0)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
                  <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--color-divider)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
                    <button type="button" className="btn btn-icon" onClick={() => updateQuantity(r.product._id, r.variant, Math.max(1, r.qty - 1))} style={{ borderRadius: 0, width: 30, height: 30 }}>–</button>
                    <span style={{ minWidth: 28, textAlign: 'center', font: '500 13px/1 Inter' }}>{r.qty}</span>
                    <button type="button" className="btn btn-icon" onClick={() => updateQuantity(r.product._id, r.variant, r.qty + 1)} style={{ borderRadius: 0, width: 30, height: 30 }}>+</button>
                  </div>
